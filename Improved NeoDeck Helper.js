// ==UserScript==
// @name         GC - Improved NeoDeck Helper
// @version      1.0
...
// original script by sanjix https://greasyfork.org/en/users/1175371-sanjix, modifications by 4leafed
...
function populateMissingCards(card) {
	...
	shopWizLink.setAttribute('href', '/market/wizard/?submit=Search&query=' + card.name + '&area=0&search_method=1&min_price=&max_price=');
	...
	searchLink.setAttribute('target', '_neodeckSearch');
	searchLink.setAttribute('href', '/search/?query=' + card.name);
	var searchImg = document.createElement('img');
	searchImg.setAttribute('width', '15px');
	searchImg.setAttribute('src', 'https://grundoscafe.b-cdn.net/searchicons/coloured/search.png');
    searchLink.style.paddingRight = '3px';

    var vpLink = document.createElement('a');
	vpLink.setAttribute('target', '_neodeckSearch');
	vpLink.setAttribute('href', 'https://virtupets.net/search?q=' + card.name);
	var vpImg = document.createElement('img');
	vpImg.setAttribute('width', '15px');
	vpImg.setAttribute('src', 'https://virtupets.net/assets/images/vp.png');
    vpLink.style.paddingRight = '3px';

	shopWizLink.appendChild(shopWizImg);
	tpLink.appendChild(tpImg);
	sdbLink.appendChild(sdbImg);
    searchLink.appendChild(searchImg);
    vpLink.appendChild(vpImg);

	li.appendChild(shopWizLink);
	li.appendChild(tpLink);
	li.appendChild(sdbLink);
    li.appendChild(searchLink);
    li.appendChild(vpLink);
